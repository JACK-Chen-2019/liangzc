...
![]({{site.baseurl}}/static/_posts/staticYogdbSnzUobrt2x0nrncsfZGnLd.png)
...
- Continual domain adaptation
  ...
  - Source data：daytime night
    ![]({{site.baseurl}}/static/_posts/staticIsE6bhJyqoYmikxhD9qcua9Jnqf.png)
- Test-time adaptation

![]({{site.baseurl}}/static/_posts/staticLyolbQLYeo39FUxn0kMcKa9Ungh.png)
...
![]({{site.baseurl}}/static/_posts/staticRyBWbwOYSoavK0xfunockjlUnGb.png)
...
- Weight-Averaged Pseudo-Labels
  ...
  - Teacher Model：保留源模型信息，采用 ema 移动平均更新。
    ![]({{site.baseurl}}/static/_posts/staticIcnKbsHMToE8gXxCeoycyFS6nYe.png)
  - Student Model（main）：对目标域数据预测与 Teacher Model 保持一致，采用 CE Loss 梯度更新。
    ![]({{site.baseurl}}/static/_posts/staticIBKQbrbcFogEIUxAsnScYqCbnje.png)
- Augmentation-Averaged Pseudo-Labels

  - 预测概率取几个 aug 上的平均值

![]({{site.baseurl}}/static/_posts/staticGbfcbrscZo22FKx4pAecGAygn6b.png)

- Stochastic Restoration：保留初始模型参数
  - 随机更新某些参数
    ![]({{site.baseurl}}/static/_posts/staticRjAqbHGjdox5ToxeVPQclsHcnbt.png)
  - 参数更新方式

![]({{site.baseurl}}/static/_posts/staticGGXzbp8qioErClxqwsGcL9TZnZf.png)
...
- Memory-efficient Architecture
  ...
  - 使用源域数据训练 lightweight meta networks
    ![]({{site.baseurl}}/static/_posts/staticRmzsbURbBoEnlLxxJuzcrMJhnXc.png)
  - Entropy minimization
    - 熵最小化，参考 tent
      ![]({{site.baseurl}}/static/_posts/staticZZLIb718XoHPv9xBE8OcKTXtnzb.png)

  ![]({{site.baseurl}}/static/_posts/staticTki9bGLFjoxck8xVnoscX3cDnMh.png)
- Self-distilled Regularization

  - $x_k$:K 个 meta network frozen 的输出
  - $\hat{x_k}$:K 个 meta network 现在的输出

![]({{site.baseurl}}/static/_posts/staticRTE8bYTCPofO3xx8HOEcWKs4n7d.png)

![]({{site.baseurl}}/static/_posts/staticWE7LbgPd7o98MHxlJikcekKJnxc.png)

![]({{site.baseurl}}/static/_posts/staticRHpbbxfXOo5VlIxoduWcVVUdnBh.png)
...
![]({{site.baseurl}}/static/_posts/staticUpsTbsGp3or4wKxb2pgcLOq1nog.png)
...
![]({{site.baseurl}}/static/_posts/staticBgLpbBCFtogLNHxzhzycCY4knCe.png)
...
- 极大似然法，$\hat{z_i}$是样本在真实分布下的预测概率[1*K]，$p_i$是源分布上的预测概率，根据测试数据来估计真实分布：
  ![]({{site.baseurl}}/static/_posts/staticCDdgbTipZoA9TfxP36NcpyRknKh.png)
  极大似然的优化目标：$argmax(\mathcal{L}(\hat(Z)))$

  ![]({{site.baseurl}}/static/_posts/staticIY6nbLJOfocvcLxQFTscFdhlnNb.png)
  优化目标：$argmin(\sum\limits_{i = 1}^N {KL({{z}_i}||{p_i})})$
  $w_{ij}$衡量样本$i$和$j$特征的相似度，即特征相似的两个样本预测结果相似。

![]({{site.baseurl}}/static/_posts/staticPdDgbxibFoVKfExW1E0cqYgfn4g.png)
...
- 每一个批量的分布对模型有影响，batch normalization，对于 continuous batchsize 为 1，因此影响更大。
  ...
  - 利用样本的均值、协方差估计，调整 batch normalization 的参数
    ![]({{site.baseurl}}/static/_posts/staticZlsIbiRS1oX5JExL8yOcc8vlnGb.png)
- 写的很不错，表达简单清楚。
-
